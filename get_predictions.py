...
if __name__ == '__main__':
    ...
    mnb_model = pickle.load(open('models/20200115_mnb_bow_v0.1.1.pickle', 'rb'))

    test_bow = pd.read_csv('Data/test/bag_of_words_v0.1.1.csv')
    y_pred = mnb_model.predict(test_bow)

    df_pred = pd.DataFrame(y_pred, index=df_test.id, columns=['label'])
    df_pred.to_csv('outputs/20200115_mnb_bow_v0.1.1.csv')
